...
const ReqRelatedPosts = ({ category, postId }) => {
  const [relatedPosts, setRelatedPosts] = useState([]);

  useEffect(() => {
    const fetchRelatedPosts = async () => {
      try {
        const res = await fetch(`http://localhost:3000/api/posts/requestPosts/relatedPosts/${category}`);
        if (!res.ok) {
...
export default ReqRelatedPosts;
